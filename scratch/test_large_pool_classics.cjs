...
// Require classics: Martini, Manhattan, Negroni, Old Fashioned
// So we must keep: gin, vermouth, whiskey, sweet-vermouth, bitters, campari, simple-syrup
const requiredDrinks = ['martini', 'manhattan', 'negroni', 'old-fashioned'];
const mustKeep = new Set();
for (const d of extendedDrinks) {
    if (requiredDrinks.includes(d.id)) {
        d.ing.forEach(i => mustKeep.add(i));
    }
}
// We also probably want vodka, rum, tequila, lime-juice so we can make other things. Let's not force it though.
const mustKeepArr = [...mustKeep];
const remainingToPick = allIngredients.filter(i => !mustKeepArr.includes(i));
const pickCount = targetCount - mustKeepArr.length;
...
for (const pick of subsetsToPick) {
    const keep = new Set([...mustKeepArr, ...pick]);
    let possibleDrinks = [];
    for (const d of extendedDrinks) {
